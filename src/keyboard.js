...
export default class SpellKeyboard {
    ...
    resetKeyboard () {
        for (let i = 0; i < this.keys.length; i++) { this.keyPress[this.keys[i]] = false; }
    }

    startListemKeyboard = () => {
        if (!this.keyPress) { this.resetKeyboard(this.keys); };
        if (typeof window.keypress.Listener === 'undefined') { throw new Error('You need to import keypress-2.1.5 library'); };
        if (this.keys.length === 0) { return false; };

        this.listener = new window.keypress.Listener();

        this.keys.forEach((key) => {
            this.listener.counting_combo(key, () => {
